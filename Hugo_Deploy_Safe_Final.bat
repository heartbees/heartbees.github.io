...
echo.
echo ========================================
echo Hugo 博客超极简部署脚本（安全稳定，零误删）
echo 核心：本地新增/删除文章 → 直接覆盖同步到GitHub
echo ========================================
echo.

rem 3. 第一步：切换到main分支（源码分支），不做任何强制操作
...
rem 5. 第三步：推送main分支到GitHub（本地修改覆盖远程，杜绝旧文章回传）
echo 【3/5】推送main分支到GitHub（本地修改覆盖远程）
git push origin main
if %errorlevel% equ 0 (
    echo 提示：✅ main分支推送成功！GitHub的content/posts已同步
) else (
    echo 提示：⚠️ main分支推送失败，尝试强制推送（解决旧文章回传）
    echo 正在执行：git push origin main -f
    git push origin main -f
    echo 提示：强制推送完成，远程旧内容已被本地覆盖
)
echo.

rem 6. 第四步：构建静态文件（只删除旧public，不碰其他文件）
...
rem 7. 第五步：切换到gh-pages分支，推送静态文件（线上更新）
...
rem 直接复制public内容，不删除旧文件（git自动识别新增/删除）
...
if %errorlevel% equ 0 (
    ...
    echo 提示：⚠️ gh-pages推送失败，尝试强制推送
    echo 正在执行：git push origin gh-pages -f
    git push origin gh-pages -f
    echo 提示：强制推送完成，线上旧内容已被本地覆盖
)
echo.

rem 8. 切回main分支，收尾提示（无强制操作，不丢失数据）
git checkout main
echo.
echo ========================================
echo 部署完成！核心效果已实现：
